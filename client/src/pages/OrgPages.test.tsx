...
describe("OrgMembersPage", () => {
  it("lists org members with editable roles for an Owner", () => {
    renderRoute("/orgs/org-acme/members");
    expect(screen.getByRole("heading", { level: 1, name: "Members" })).toBeInTheDocument();
    expect(screen.getByRole("cell", { name: "sarah.chen@acme.com" })).toBeInTheDocument();
    // current user (Alex) is an Owner -> can remove others
    expect(screen.getByRole("button", { name: "Remove Marcus Vance" })).toBeInTheDocument();
  });

  it("changes a member's role", async () => {
    renderRoute("/orgs/org-acme/members");
    const row = screen.getByText("Marcus Vance").closest("tr")!;
  ...
  it("removes a member", async () => {
    renderRoute("/orgs/org-acme/members");
    await userEvent.click(screen.getByRole("button", { name: "Remove David Kim" }));
    expect(screen.queryByRole("cell", { name: "david.kim@acme.com" })).not.toBeInTheDocument();
  });

  it("invites a new member through the modal", async () => {
    renderRoute("/orgs/org-acme/members");
    await userEvent.click(screen.getByRole("button", { name: "+ Invite Member" }));
    await userEvent.type(screen.getByLabelText("Email Address"), "new.hire@acme.com");
    await userEvent.click(screen.getByRole("button", { name: "Send Invite" }));
    expect(await screen.findByRole("cell", { name: "new.hire@acme.com" })).toBeInTheDocument();
  });
});

describe("OrgSettingsPage", () => {
  it("renders the name form and a disabled Danger Zone delete", () => {
    renderRoute("/orgs/org-acme/settings");
    expect(screen.getByRole("heading", { level: 1, name: "Organization Settings" })).toBeInTheDocument();
    expect(screen.getByLabelText("Organization Name")).toHaveValue("Acme Design Studio");
  ...
  it("saves a new organization name and shows a toast", async () => {
    ...
    // nav org switcher reflects the rename
    expect(screen.getByRole("button", { name: /Acme Studios/ })).toBeInTheDocument();
  });
